...
//        var image:Bitmap? = null
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(resultCode == RESULT_OK){
            when(requestCode){
                REQ_CAMERA -> {
//                    val bitmap = data?.extras?.get("data") as Bitmap //미리보기 이미지. 깨질 수 있음.
//                    binding.imagePreview.setImageBitmap(bitmap)
                ...
                REQ_GALLERY -> {
                    //data의 data에 uri형태로 이미지 주소가 담겨있다.
                    //setImageBitmap할 필요가 없는게, 저장과정이 필요없기 때문
                    data?.data.let{ uri ->
